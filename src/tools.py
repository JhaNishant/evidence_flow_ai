...
def create_calculator_tool(logger: ToolLogger):
    """Create a calculator that accepts only basic numeric arithmetic."""

    allowed_binary_operations = (ast.Add, ast.Sub, ast.Mult, ast.Div, ast.Mod, ast.Pow)
    allowed_unary_operations = (ast.UAdd, ast.USub)

    def validate_expression(node: ast.AST) -> None:
        if isinstance(node, ast.Expression):
            validate_expression(node.body)
        elif isinstance(node, ast.BinOp) and isinstance(node.op, allowed_binary_operations):
            if (
        ...
        elif isinstance(node, ast.UnaryOp) and isinstance(node.op, allowed_unary_operations):
            validate_expression(node.operand)
        elif isinstance(node, ast.Constant) and type(node.value) in (int, float):
            return
        else:
            raise ValueError("Only numbers, parentheses, and +, -, *, /, %, and ** are allowed.")

    @tool
    def calculator(expression: str) -> str:
        """Safely evaluate a basic mathematical expression and return its result as text.
        ...
        try:
            if not expression or not expression.strip():
                raise ValueError("A mathematical expression is required.")
            if len(expression) > 250:
                raise ValueError("Expressions must be 250 characters or fewer.")

            normalized_expression = expression.replace("$", "")
            normalized_expression = re.sub(r"(?<=\d),(?=\d{3}(?:\D|$))", "", normalized_expression)
            parsed_expression = ast.parse(normalized_expression, mode="eval")
            validate_expression(parsed_expression)
            result = eval(compile(parsed_expression, "<calculator>", "eval"), {"__builtins__": {}}, {})

            if not isinstance(result, (int, float)) or isinstance(result, bool) or not math.isfinite(result):
                raise ValueError("The expression must produce a finite number.")

            formatted_result = str(int(result)) if float(result).is_integer() else format(result, ".12g")
            logger.log_tool_use(
                "calculator",
                {"expression": expression, "normalized_expression": normalized_expression},
                {"result": formatted_result},
            )
            return formatted_result
        except Exception as error:
            error_message = f"Calculation error: {error}"
            logger.log_tool_use("calculator", {"expression": expression}, {"error": error_message})
